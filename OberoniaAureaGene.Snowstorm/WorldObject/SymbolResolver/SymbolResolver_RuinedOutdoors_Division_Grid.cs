...
public class SymbolResolver_RuinedOutdoors_Division_Grid : SymbolResolver
{
    private class Child
    {
    ...
    private void FillOptions(List<Pair<int, int>> outOptions, int length)
    {
        ...
        {
            for (int j = MinPathwayWidth; j <= MaxPathwayWidth; j++)
            {
    ...
    private void ResolveOption(int roomsPerRowX, int pathwayWidthX, int roomsPerRowZ, int pathwayWidthZ, ResolveParams rp)
    {
        ...
        {
            CellRect rect = new(rp.rect.minX + num, rp.rect.minZ, pathwayWidthX, rp.rect.Height);
        ...
        {
            CellRect rect2 = new(rp.rect.minX, rp.rect.minZ + num2, rp.rect.Width, pathwayWidthZ);
        ...
        for (int k = 0; k < roomsPerRowX; k++)
        {
            for (int l = 0; l < roomsPerRowZ; l++)
            {
                Child child = new()
                {
                    rect = new CellRect(rp.rect.minX + num, rp.rect.minZ + num2, roomSize, roomSize2),
                    gridX = k,
                    gridY = l
                };
                children.Add(child);
        ...
        for (int m = 0; m < children.Count; m++)
        {
            ...
            BaseGen.symbolStack.Push("oagene_RuinedOutdoors", resolveParams4);
        }
    }
    ...
    private void MergeRandomChildren()
    {
        ...
        int num = GenMath.RoundRandom(children.Count / 6f);
        for (int i = 0; i < num; i++)
        {
            ...
            if (child is not null)
            {
                ...
                if (child2 is not null)
                {
                    ...
                    Child child3 = new()
                    {
                        gridX = Mathf.Min(child.gridX, child2.gridX),
                        gridY = Mathf.Min(child.gridY, child2.gridY),
                        merged = true,
                        rect = CellRect.FromLimits(Mathf.Min(child.rect.minX, child2.rect.minX), Mathf.Min(child.rect.minZ, child2.rect.minZ), Mathf.Max(child.rect.maxX, child2.rect.maxX), Mathf.Max(child.rect.maxZ, child2.rect.maxZ))
                    };
                    children.Add(child3);
                }
                continue;
